...
function bindContextMenuOnce() {
    ...
    document.getElementById('ctx-toggle').addEventListener('click', async () => {
        ...
        if (!step) return; hide();
        try {
            const raw = await Api.patch(`/steps/${ctxTargetId}`, { enabled: !step.enabled });
            const updated = raw?.publicId ? raw : (raw?.data ?? raw);
...
async function runScenario() {
    ...
    try {
        ...
        if (!execPublicId) {
            throw new Error('Execution ID alınamadı: ' + JSON.stringify(response));
        }

        Toast.success('Senaryo kuyruğa alındı!');

        setTimeout(() => {
            window.location.href = `execution-detail.html?id=${execPublicId}`;
        }, 800);
