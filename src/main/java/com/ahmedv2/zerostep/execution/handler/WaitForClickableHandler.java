...
public class WaitForClickableHandler implements ActionHandler {
    private final SelectorResolver selectorResolver;

    @Override public ActionType supports() { return ActionType.WAIT_FOR_CLICKABLE; }
    ...
    public void execute(TestStep step, ExecutionContext context) {
        ...
        context.logInfo("Wait for clickable (max " + s + "s): " + by);
        WaitUtil.waitClickable(context.getDriver(), by, s);
    }
}