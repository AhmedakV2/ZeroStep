...
public class AssertVisibleHandler implements ActionHandler {
    private final SelectorResolver selectorResolver;

    @Override public ActionType supports() { return ActionType.ASSERT_VISIBLE; }
    ...
    public void execute(TestStep step, ExecutionContext context) {
        ...
        try {
            ...
            if (!el.isDisplayed()) {
                throw new AssertionError("Element bulundu ama gorunmuyor: " + by);
            }
            context.logInfo("ASSERT visible OK: " + by);
        } catch (org.openqa.selenium.TimeoutException e) {
            throw new AssertionError("Element gorunmuyor: " + by);
        }
    }
}