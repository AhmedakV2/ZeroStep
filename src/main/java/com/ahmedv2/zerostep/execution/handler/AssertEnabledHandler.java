...
public class AssertEnabledHandler implements ActionHandler {
    private final SelectorResolver selectorResolver;

    @Override public ActionType supports() { return ActionType.ASSERT_ENABLED; }
    ...
    public void execute(TestStep step, ExecutionContext context) {
        ...
        if (!el.isEnabled()) {
            throw new AssertionError("Element disabled (enabled bekleniyordu): " + by);
        }
        context.logInfo("ASSERT enabled OK");
    }
}